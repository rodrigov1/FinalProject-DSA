...
template <class T> class Lista {
  ...
  public:
	...
	void addOrdenado(T d) { addO(d, nullptr); };  // sumar nodos a la lista Ordenados de menor a MAYOR
...
template <class T> void Lista<T>::addO(T d, Nodo<T> *ant) {
	if (this->esvacia()) { // el primero
		...
		if (d < czo->get_dato()) {
			...
			if ((czo->get_next())->get_next() == nullptr) { // al final
				Nodo<T> *nuevo = new Nodo<T>(d);
...
template <class T> void Lista<T>::addFinal(T d) {
	if (this->esvacia()) { // el primero
		this->add(d);
	} else {
		if ((czo->get_next())->get_next() == nullptr) { // al final
...
template <class T> T Lista<T>::search_id(int id) {
	if (this->esvacia()) {
		return NULL;
	} else {
