		...
		for (int i = 0; i < canales_vuelta->size(); i++)
		{
			...
			while (cant_lim > 0) // Mientras haya ancho de banda disponible
			{
				...
				if (pkg != NULL)
				{
					...
					{ // en caso que deba seguir de viaje el paquete
...
void Router::print_outPackets()
{
	...
	{
		cout << aux->get_dato()->getId() << "              " << aux->get_dato()->getOrigen()[0] << ":"
			 << aux->get_dato()->getOrigen()[1] << "       " << aux->get_dato()->getDestino()[0] << ":"
...
void Router::print_inPackets()
{
	...
	for (int i = 0; i < inPackets->size(); i++)
	{
		...
		cout << aux->get_dato()->getId() << "              " << aux->get_dato()->getOrigen()[0] << ":"
			 << aux->get_dato()->getOrigen()[1] << "       " << aux->get_dato()->getDestino()[0] << ":"
			 << aux->get_dato()->getDestino()[1] << "        " << aux->get_dato()->getPageId() << "            ["
			 << actual << "/" << total << "]" << endl;
...
void Router::print_rutas()
{
	...
	{
		cout << aux->get_dato()->getNext() << "          " << aux->get_dato()->getLast() << "          "
			 << aux->get_dato()->getDistancia() << endl;
		aux = aux->get_next();
	}
	cout << endl;
}
